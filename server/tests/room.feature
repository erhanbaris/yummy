Feature: Authentication features
  ...
  Scenario: Create basic room
    ...
    Then user1 authenticated
    And user2 authenticated
    And user3 authenticated
    And user4 authenticated
    And user5 authenticated
    ...
    And user1 receive UserJoinedToRoom message

    When user3 try to join room1 as User
    Then user3 joined to ""
    And user1 receive UserJoinedToRoom message
    And user2 receive UserJoinedToRoom message

    When user4 try to join room1 as User
    Then user4 joined to ""
    And user1 receive UserJoinedToRoom message
    And user2 receive UserJoinedToRoom message
    And user3 receive UserJoinedToRoom message
