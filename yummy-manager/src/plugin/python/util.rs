...
/* *************************************************** IMPORTS **************************************************** */
/* **************************************************************************************************************** */
use yummy_model::meta::{UserMetaAccess, UserMetaType, MetaType};
...
/* **************************************************************************************************************** */
impl MetaTypeUtil {
    pub fn parse(vm: &VirtualMachine, obj: &PyObjectRef, access: UserMetaAccess) -> Result<UserMetaTypeWrapper, PyRef<PyBaseException>> {
        ...
        if obj.class().fast_issubclass(vm.ctx.types.list_type) {
            ...
            for item in python_list.borrow_vec().iter() {
                meta_list.push(Self::parse(vm, item, UserMetaAccess::User)?.data);
            }
    ...
    pub fn as_python_value(meta: &UserMetaType, vm: &VirtualMachine) -> PyResult<PyObjectRef> {
        Self::inner_as_python_value(meta, vm)
    }

    fn inner_as_python_value(meta: &UserMetaType, vm: &VirtualMachine) -> PyResult<PyObjectRef> {
        match meta {
    ...
    pub fn get_meta_type(meta: &UserMetaType, vm: &VirtualMachine) -> PyResult<PyObjectRef> {
        match meta {
