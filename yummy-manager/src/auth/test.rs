...
fn create_actor(config: Arc<YummyConfig>) -> anyhow::Result<(Addr<AuthManager<yummy_database::SqliteStore>>, Arc<DummyClient>)> {
    ...
    Ok((AuthManager::<yummy_database::SqliteStore>::new(config.clone(), states, connection, executer).start(), Arc::new(DummyClient::default())))
}

/* email unit tests */
#[actix::test]
async fn create_user_via_email() -> anyhow::Result<()> {
    let (address, socket) = create_actor(yummy_model::config::get_configuration())?;
