...
use yummy_model::auth::validate_auth;
...
use actix::Addr;
use anyhow::Ok;
use yummy_database::{create_database, create_connection};
...
fn create_actor() -> anyhow::Result<(Addr<RoomManager<yummy_database::SqliteStore>>, Addr<AuthManager<yummy_database::SqliteStore>>, Arc<YummyConfig>, YummyState, Arc<DummyClient>)> {
    ...
    Ok((RoomManager::<yummy_database::SqliteStore>::new(config.clone(), states.clone(), connection.clone(), executer.clone()).start(), AuthManager::<yummy_database::SqliteStore>::new(config.clone(), states.clone(), connection.clone(), executer).start(), config, states.clone(), Arc::new(DummyClient::default())))
}
...
async fn message_to_room() -> anyhow::Result<()> {
    ...
    assert_eq!(message.user_id, user_2_id.clone());
    assert_eq!(message.room_id, room_id.clone());
...
macro_rules! message_received_from_room {
    ($socket: expr, $sender: expr, $room_id: expr, $message: expr) => {
        ...
        let message = serde_json::from_str::<MessageReceivedFromRoom>(&message).unwrap();
        assert_eq!(message.user_id, $sender.clone());
        assert_eq!(message.room_id, $room_id.clone());
...
async fn multi_room_support() -> anyhow::Result<()> {
    ...
    let auth_manager = AuthManager::<yummy_database::SqliteStore>::new(config.clone(), states.clone(), connection.clone(), executer.clone()).start();
    let room_manager = RoomManager::<yummy_database::SqliteStore>::new(config.clone(), states.clone(), connection.clone(), executer.clone()).start();
...
async fn room_join_request_approve() -> anyhow::Result<()> {
    ...
    room_manager.send(ProcessWaitingUser {
        ...
        auth: user_1_auth.clone(),
        room_id: room_1_id,
        user_id: user_2_auth_jwt.id.deref().clone(),
        status: true,
        socket: user_1_socket.clone()
    }).await??;
...
async fn user_ban_test() -> anyhow::Result<()> {
    ...
    room_manager.send(KickUserFromRoom {
        ...
        auth: user_2_auth.clone(),
        room_id: room_1_id,
        user_id: user_1_auth_jwt.id.deref().clone(),
        socket:user_2_socket.clone(),
        ban: true
    }).await?.unwrap_err();
